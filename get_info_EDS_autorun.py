...
main_dirs = [i for i in os.listdir(path) if os.path.isdir(i) and '!' not in i and i not in [j for j in ('ХЗ','смена фамилии', 'Заявы', 'Заявления', 'backup_sent_EDS')]]
...
for n, i in enumerate(main_dirs):
	...
	eds[i] = {'name':link,
				...
				'refresh':'',
				'get_not_sign':'',
				'repeated_container':'=ЕСЛИ(СЧЁТЕСЛИ($F$2:$F$1734; F%s)>1;"Дубликат";"")' % str(n + 2),
				'remained_true': '=ЕСЛИ(I%s<>"";ЕСЛИ(I%s<=45;ИСТИНА;"");"")' % (str(n + 2),str(n + 2))}
	...
	for j in list_files:
		...
		if j.endswith('.txt'):
			try:
				...
				eds[i]['password'] = ''
	try:
		with open (i + '/' + eds[i]['eds_dir'] + '/name.key', encoding='cp1251') as f:
